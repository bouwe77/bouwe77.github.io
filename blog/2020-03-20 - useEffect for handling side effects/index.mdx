...
React offers several hooks and one of them you'll encounter almost certainly, even when building
the most basic app, and that is the `useEffect` hook.

It is not only very commonly used, it's also quite challenging to really grasp it.
...
a user (event), it is a _side effect_ for a React component: It does not happen within the cycle of rendering
and handling events.
...
_when_ to execute these side effects. This is done by supplying a _second_ argument to `useEffect`, which is the so-called _dependencies array_.
...
or variables (so functions too) that are defined _inside_ the component.
...
So our side effect has no dependencies, but how can we use the dependencies array to make sure the effect is only called once?

Before the side effect inside `useEffect` is executed, React checks first whether the variables in the dependencies array have
...
So to make sure an effect is only executed once, you just make sure the values inside the dependencies array _never_ change.
And the easiest way to achieve that is to pass an _empty_ dependencies array because an empty array will never change! 😄
...
I'll import the `getNumberText` which handles calling the Numbers API and I remove the import of `getRandomText`.
...
the moment to add the `clicked` variable to the dependencies array:
...
>
> Then read Dan Abramow's very long blog post (or mini-book, as he calls it)
